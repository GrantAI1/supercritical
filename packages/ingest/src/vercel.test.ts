...
describe("resolveVercelAccount", () => {
    it("prefers team id", () => {
        expect(
            resolveVercelAccount(envelope("deployment.created", DEPLOY_PAYLOAD))
        ).toBe("team_x");
    });
    it("falls back to user id", () => {
        expect(
            resolveVercelAccount(
                envelope("deployment.created", {
                    ...DEPLOY_PAYLOAD,
                    team: null,
                })
            )
...
describe("normalizeVercel", () => {
    it("production deployment.succeeded is a change point", () => {
        const evt = normalizeVercel(
            envelope("deployment.succeeded", DEPLOY_PAYLOAD)
        );
    ...
    it("deployment.error is ERROR severity", () => {
        const evt = normalizeVercel(
            envelope("deployment.error", DEPLOY_PAYLOAD)
        );
        expect(evt).toMatchObject({
    ...
    it("unknown type returns null", () => {
        expect(
            normalizeVercel(envelope("integration-configuration.removed", {}))
        ).toBeNull();
    });
