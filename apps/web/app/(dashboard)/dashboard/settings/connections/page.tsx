...
export default async function ConnectionsPage() {
    ...
    return (
        <div className="flex flex-col gap-6">
            <h1 className="text-sm tracking-[0.2em] text-terminal-amber">
                CONNECTIONS
            </h1>
            <table className="w-full text-left text-xs">
                ...
                <tbody>
                    {connections.length === 0 && (
                        <tr>
                            <td colSpan={4} className="py-3 text-terminal-dim">
                                NO CONNECTIONS — add one below, then point the
                                provider webhook at the URL.
                            </td>
                        </tr>
                    )}
                    {connections.map((c: any) => (
                        <tr
                            key={c.id}
                            className="border-b border-terminal-grid hover:bg-neutral-900/60"
                        >
                            ...
                            <td className="py-1 text-terminal-dim">
                                /api/webhooks/{c.provider.toLowerCase()}
                            </td>
                        </tr>
                    ))}
                </tbody>
