        ...
        <div className="flex flex-col text-xs">
            <div className="flex items-center justify-between border-b border-terminal-grid pb-1">
                ...
                {degraded ? (
                    ...
                    <span className="flex items-center gap-1 text-terminal-dim">
                        <span className="live-dot text-terminal-green">●</span>{" "}
                        LIVE · 5s
                    </span>
                )}
            </div>
            {events.length === 0 ? (
                <p className="py-4 text-terminal-dim">
                    NO EVENTS YET — configure a webhook in settings →
                    connections.
                </p>
            ) : (
                <table className="w-full">
                    <tbody>
                        {events.map((e) => {
                            const src = SOURCE_TAGS[e.source] ?? {
                                tag: e.source.slice(0, 3),
                                class_name: "",
                            };
                            return (
                                <tr
                                ...
                                >
                                    ...
                                    <td
                                        className={`w-10 pr-2 ${src.class_name}`}
                                    >
                                        {src.tag}
                                    </td>
                                    <td className="w-56 truncate pr-2 text-terminal-dim">
                                        {e.kind}
                                    </td>
                                    <td
                                        className={`truncate pr-2 ${SEVERITY_CLASSES[e.severity] ?? ""}`}
                                    >
                                        {e.is_change_point ? "◆ " : ""}
                                        {e.title}
                                    </td>
                                    <td className="w-32 truncate text-right text-terminal-dim">
                                        {e.service}
                                    </td>
                                </tr>
                            );
